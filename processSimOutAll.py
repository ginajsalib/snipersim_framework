...
#        print(content)
        ...
        instructions_pattern = re.compile(r'Instructions\s*\|\s*(\d+)\s*\|\s*(\d+)')
        matches_instructions = instructions_pattern.findall(content)
        if matches_instructions:
             instructions = [int(match.replace(',', '').strip()) for match in matches_instructions[0]]
        print("Instructions for each core found: ", instructions)

    return branch_mispredictions, l2_cache_misses, l3_cache_misses, time_ns, cycles, instructions
...
def traverse_and_parse(base_directory):
    ...
    print("Processing directory .. {}".format(base_directory)) 
        ...
        if benchmark in os.path.basename(root):
           for file in files:
                if file == 'sim.out':
                    ...
                    branch_mispredictions, l2_cache_misses, l3_cache_misses, time_ns, cycles, instructions = parse_sim_out(file_path)
...
def save_to_csv(data, filename):
    with open(filename, 'w') as csvfile:  # 'wb' mode for Python 2
        ...
        for dir_path, row in data.items():
            max_len = max(max_len, len(row['branch_mispredictions']), len(row['l2_cache_misses']), len(row['l3_cache_misses']), len(row['time_ns']), len(row['cycles']), len(row['instructions']))
        ...
        for i in range(max_len):
            ...
            fieldnames.append('instructions_{}'.format(i+1))
        ...
        for dir_path, row in data.items():
            ...
            instructions = row['instructions'] + [''] * (max_len - len(row['instructions']))
            ...
            for i in range(max_len):
                ...
                row_dict['instructions_{}'.format(i+1)] = instructions[i]
